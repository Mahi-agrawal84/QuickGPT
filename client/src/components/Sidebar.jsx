...
//       <div className='flex-1 overflow-y-auto no-scrollbar mt-3 text-sm space-y-3'>
//         {
...
//           key={chats._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md
...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const { chats, setSelectedChat, theme, setTheme, user, logout, deleteChat, navigate } = useAppContext()
  const [search, setSearch] = useState('')
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 bg-white dark:bg-[#09090b] border-r border-gray-200 dark:border-white/5 transition-all duration-500 max-md:fixed left-0 top-0 bottom-0 z-50 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* New Chat Button */}
      <button className='flex justify-center items-center w-full py-2 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm font-medium rounded-lg cursor-pointer shadow-sm active:scale-95 transition-all'>
